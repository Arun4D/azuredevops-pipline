...
GetRepoResponse=$(curl --location --request GET "https://dev.azure.com/$organization/$projectName/_apis/git/repositories?api-version=7.1-preview.1" -H 'Authorization: Basic <<PAT encode>>' -H 'Cache-Control: no-cache')
...
if [ "$isNameExists" -eq 0 ]; then
    ...
    echo ""
    CreateRepoResponse=$(curl --location --request POST "https://dev.azure.com/$organization/$project/_apis/git/repositories?api-version=7.1-preview.1" -H 'Authorization: Basic <<PAT encode>>' -H 'Cache-Control: no-cache' -H 'Content-Type: application/json' --data '{"name":"'${checkName}'","project": {  "id": "'${projectId}'" } }')
